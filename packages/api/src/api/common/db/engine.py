...
def create_async_engine(*, dsn: str, debug: bool = False) -> AsyncEngine:
  return _create_async_engine(dsn, echo=debug, json_serializer=json_serializer)


def create_sync_engine(*, dsn: str, debug: bool = False) -> Engine:
  return _create_engine(dsn, echo=debug)
