...
class CORSMatcherMiddleware:
  def __init__(self, app: ASGIApp, *, configs: Sequence[CORSConfig]) -> None:
    self.app = app
    self.config_middlewares = tuple((config, config.get_middleware(app)) for config in configs)
  ...
  def _get_config_middleware(self, origin: str, scope: Scope) -> CORSMiddleware | None:
